...
(function ( $j ) {
	...
	$j.fn.bootstrapSwitch = function( options ) {
		...
		return this.each(function(e) {
			...
			var div = $j('<div class="btn-group btn-toggle" style="white-space: nowrap;"></div>').insertAfter(this);
			var on = $j('<button class="btn btn-primary '+settings.size+disabled+'" style="float: none;display: inline-block;"></button>').html(settings.on).css('margin-right', '0px').appendTo(div);
			var off = $j('<button class="btn btn-danger '+settings.size+disabled+'" style="float: none;display: inline-block;"></button>').html(settings.off).css('margin-left', '0px').appendTo(div);
			...
			$j(this).hide().on('change', function() {
				applyChange(c.is(':checked'))
			});
		});
	};
